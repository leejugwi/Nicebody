...
def draw_skeleton(
        img, instance_scores, keypoint_scores, keypoint_coords,
        min_pose_confidence=0.5, min_part_confidence=0.5, thickness=1):
    out_img = img
    adjacent_keypoints = []
    for ii, score in enumerate(instance_scores):
        if score < min_pose_confidence:
    ...
    # 스켈레톤의 색상을 빨간색 (0, 0, 255)으로 변경하고 두껍게 그립니다.
    out_img = cv2.polylines(out_img, adjacent_keypoints, isClosed=False, color=(0, 0, 255), thickness=thickness)

    return out_img


def draw_skel_and_kp(
        img, kpt_with_conf, conf_thres=1, color=(0, 255,0), thickness=2):
    ...
    out_img = cv2.polylines(out_img, adjacent_keypoints, isClosed=False, color=color, thickness=thickness)
